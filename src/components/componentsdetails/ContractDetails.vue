<!--
@file 合同列表管理页面，展示所有合同信息并支持增删改查及批量操作
@component ContractDetails
@usedBy
  - views/ContractDetails.vue: 通过 router-view 渲染合同列表
@dependsOn
  - composables/useSmartListConfig: 列表配置
  - stores/contractStore: 合同数据管理
      ...
      >
        ...
        <template #default="slotProps">
          ...
          >
            ...
            <!-- 自定义插槽：合同金额（格式化）-->
            <template #contract_amount="{ row }">
              <span>{{ formatPrice(row.contract_amount) }}</span>
            </template>

            <!-- 自定义插槽：签订日期（格式化）-->
            <template #contract_start_date="{ row }">
              <span>{{ formatDate(row.contract_start_date) }}</span>
            </template>
            ...
            <!-- 自定义插槽：结算价格（格式化）-->
            <template #settlemented_price="{ row }">
              <span>{{ formatPrice(row.settlemented_price) }}</span>
            </template>
          </CommonList>
...
const handleExportExcel = async () => {
  const exportColumns: ColumnConfig<Contract>[] = [
    ...
    { title: '签订日期', key: 'contract_start_date', default: '' },
    ...
    { title: '初验日期', key: 'initial_check_date', default: '' },
    { title: '终验日期', key: 'final_check_date', default: '' },
    { title: '已支付金额', key: 'amount_paid', default: '0' },
