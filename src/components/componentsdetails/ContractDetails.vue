<!--
  ...
  4. 删除后使�?smartListRef.value?.refresh() 刷新
  5. 移除 isDataLoaded �?onMounted 中的 getList() 调用
...
const columns: TableColumn[] = [
  ...
  { prop: 'contract_supplier', label: '供应�?, width: 150, align: 'left' },
  {
    type: 'custom',
 ...
 *
 * 包含�? * - getList: 获取列表数据的方�? * - pagination: 分页状态（使用 getter/setter 实现双向绑定�? * - list: 列表数据（computed�? * - loading: 加载状态（computed�? * - search: 搜索配置（后端搜索）
 */
const storeConfig: PaginationSearchConfig<Contract> = {
  ...
  /**
   * 搜索配置
   * 【优化】改为后端搜索，支持多字段搜�?   * 统一使用 contractStore.getList({ search: keyword })
   */
  search: {
...
// ===== 路由监听：控制子路由遮罩 =====
/**
 * 监听路由变化，判断是否需要显示子路由遮罩�? * 当访问子路由（如新增/编辑页）时显示遮�? */
watch(
  () => route.matched,
  (matched) => {
...
/**
 * 删除合同（含二次确认�? * 【架构优化】删除成功后通过 SmartListContainer 刷新列表，保持数据一致�? * @param row 行数�? */
...
const handleBatchDelete = async (rows: Contract[] | undefined) => {
  ...
  // 提取选中的唯一标识字段（根据实体类型调整字段名�?  const codes = rows.map((row) => row.contract_code).filter((code): code is string => !!code)
...
/**
 * 导出 Excel（支持当前页/全部数据�? * @param row 行数�? */
const handleExportExcel = async () => {
  const exportColumns: ColumnConfig<Contract>[] = [
    ...
    { title: '保修�?, key: 'contract_warranty_period', default: '' },
    {
      title: '结算状�?,
    ...
    { title: '已支付金�?, key: 'contract_paid_price', default: '0' },
    { title: '已支付记�?, key: 'contract_paid_record', default: '0' },
  ]
  ...
  await exportToExcel({
    ...
    fileName,
    sheetName: '合同列表',
    confirmMessage: `确定要导�?${exportData.length} 条合同数据吗？`,
    emptyMessage: '暂无合同数据可导�?,
...
// 导入公共样式 mixin 库（使用现代 @use 语法�?@use '@/assets/styles/common-forms.scss' as *;
...
// 响应式设�?@include responsive-design;
</style>
