<!--
@file 出库资产列表页面，展示出库资产记录并支持增删改查及批量操作
@component OutAssetDetails
@usedBy
  - views/OutAssetDetails.vue: 通过 router-view 渲染出库资产列表
@dependsOn
  - composables/useOutAssetDetailCards: 出库资产详情卡片配置
  - composables/useSmartListConfig: 列表配置
  - stores/outAssetStore: 出库资产数据管理
...
defineOptions({ name: 'OutAssetDetails' })
...
import type { OutAsset, OutAssetDetail } from '@/types/outasset'
...
.table-container {
  ...
  /* 本页差异：宽松 padding + 长文本断词换行——外层变量覆盖 --table-* 令牌，
     CSS 变量跨 scoped 边界继承至内层 th/td（C-10 收敛，替代 :deep + !important） */
  --table-header-padding: 16px 12px;
  --table-body-padding: 12px 8px;
  --table-header-word-break: break-word;
  --table-body-word-break: break-word;
}
