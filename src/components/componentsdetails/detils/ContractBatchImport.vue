    ...
    <el-card class="box-card">
      ...
      <div v-if="previewData.length > 0" class="preview-table">
        ...
        <el-table :data="paginatedPreviewData" border stripe max-height="500">
          ...
          <el-table-column label="供应�? prop="data.contract_supplier" width="150" />
          ...
          <el-table-column label="结算状�? prop="data.contract_settledment_status" width="100" />
...
const importConfig: BatchImportConfig<ContractExcelRow, ContractCreateForm> = {
  ...
  validateItem: (item: ContractExcelRow) => {
    ...
    if (!item.contract_supplier?.trim()) {
      errors.contract_supplier = '供应商不能为�?
    }
    ...
    // 签订日期格式校验
    // Excel 解析后的日期可能�?Date 对象或字符串，需要统一转换为字符串
    ...
    // 保修期校�?    const warranty = Number(item.contract_warranty_period)
    if (isNaN(warranty) || warranty < 0) {
      errors.contract_warranty_period = '保修期必须是有效数字且不小于0'
    }

    // 结算状态校�?    const validStatuses = ['pending', 'settled']
    if (!item.contract_settledment_status?.trim()) {
      errors.contract_settledment_status = '结算状态不能为�?
    ...
    if (item.contract_settledment_price !== undefined && item.contract_settledment_price !== '') {
      ...
      if (isNaN(settlementPrice) || settlementPrice < 0) {
        errors.contract_settledment_price = '结算价格必须是有效数字且不小�?'
      }
    }
    ...
    if (item.contract_paid_count_number !== undefined && item.contract_paid_count_number !== '') {
      ...
      if (isNaN(paidCount) || paidCount < 0 || !Number.isInteger(paidCount)) {
        errors.contract_paid_count_number = '已付次数必须是非负整�?
      }
    }
    ...
    if (item.contract_paid_price !== undefined && item.contract_paid_price !== '') {
      ...
      if (isNaN(paidPrice) || paidPrice < 0) {
        errors.contract_paid_price = '已付金额必须是有效数字且不小�?'
      }
    }
  ...
  transformToApiData: (row: ContractExcelRow): ContractCreateForm => ({
    ...
    // Excel 解析后的日期可能�?Date 对象，需要转换为字符�?    contract_signing_date:
  ...
  // placeholder: 实际提交逻辑�?handleSubmit 中直接调�?batchCreateContracts
...
const handleUploadChange = async (uploadFile: UploadFile, uploadFileList: UploadFile[]) => {
  ...
  // 调用批量导入的核心处理函数（内部会设�?parseError �?previewData�?  await rawHandleFileChange(rawFile)
  // 文件解析完成后重置分页到第一�?  resetPreviewPage() // 新文件加载后重置页码
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 使用 index 匹配：后�?fail_items �?index 对应 validRows 的数组索�?    if (result.fail_count > 0) {
    ...
    if (result.fail_count === 0) {
      ...
      // 通知父页面刷新数�?      contractStore.setRefreshFlag(true)
    } else {
      ElMessage.warning(
        `导入完成：成�?${result.success_count} 条，失败 ${result.fail_count} 条`,
      )
    }
  } catch (error) {
