  ...
  >
    ...
    <template v-if="!loading">
      <template v-if="boundEmployee">
        <el-descriptions :column="2" border size="small">
          <el-descriptions-item label="工号">{{ boundEmployee.employee_jobcode }}</el-descriptions-item>
          <el-descriptions-item label="姓名">{{ boundEmployee.employee_name }}</el-descriptions-item>
          <el-descriptions-item label="状态">{{ boundEmployee.employee_status }}</el-descriptions-item>
          <el-descriptions-item label="绑定用户">{{ boundEmployee.auth_user_username || '无' }}</el-descriptions-item>
        </el-descriptions>
        <div class="action-row">
          <el-button type="danger" plain :loading="actionLoading" @click="handleUnbind">解绑</el-button>
          <el-button type="warning" plain :loading="actionLoading" @click="showSearch = true">替换</el-button>
        </div>
      </template>
      <template v-else>
    ...
    <template #footer>
      ...
      <template v-if="!boundEmployee && mode === 'from-authuser'">
        <el-button type="primary" :disabled="!selectedEmployee" :loading="actionLoading" @click="handleBind">
          绑定
        </el-button>
      </template>
      <template v-else-if="showSearch">
        <el-button type="primary" :disabled="!selectedEmployee" :loading="actionLoading" @click="handleReplace">
          确认替换
        </el-button>
      </template>
    </template>
...
const onSearchInput = () => {
  ...
  searchTimer = setTimeout(async () => {
    try {
      searchResults.value = await authUserAPI.searchEmployees(searchKeyword.value.trim())
    } catch {
      searchResults.value = []
    } finally {
...
const loadBindStatus = async () => {
  if (props.mode === 'from-authuser' && props.authUser) {
    loading.value = true
    try {
      boundEmployee.value = await authUserAPI.getBoundEmployee(props.authUser.id)
...
const handleBind = async () => {
  if (!selectedEmployee.value || !props.authUser) return
  actionLoading.value = true
  try {
    await authUserAPI.bindAuthUser(selectedEmployee.value.employee_jobcode, props.authUser.id)
...
const handleReplace = async () => {
  ...
  actionLoading.value = true
  try {
    await authUserAPI.replaceAuthUser(selectedEmployee.value.employee_jobcode, props.authUser.id)
