 ...
 * 用于创建回收资产时的表单数据
 *
 * 字段映射（前竀ↀ后端 RecycleAssetSerializer）：
 * - outasset_recordcode ↀrecycle_outasset (SlugRelatedField, slug_field="recordcode")
 * - recycle_asset_storage_code ↀrecycle_asset_storage (SlugRelatedField, slug_field="storage_code")
 * - recycle_asset_recycle_person_jobcode ↀrecycle_asset_recycle_person_jobcode (SlugRelatedField)
 * - recycle_type: 回收原因（新增字段）
 */
export interface RecycleAssetCreateForm {
  /** 出库记录编码 (ↀrecycle_outasset, SlugRelatedField) */
  outasset_recordcode: string
  /** 回收资产编码 (FK ↀam_asset.recordcode) */
  recycle_asset: string
  /** 回收数量 (default=1) */
  recycle_asset_number: number
  /** 回收仓库编码 (ↀrecycle_asset_storage, SlugRelatedField) */
  recycle_asset_storage_code: string
  /** 回收人工号(ↀrecycle_asset_recycle_person_jobcode, SlugRelatedField) */
 ...
 * 使用人信息通过序列化器 FK 铀read_only 字段自动获取（using_person_name / using_person_jobcode＀ */
export interface RecycleAsset {
  /** 主键 ID */
  id: number
  /** 后端记录编码 (与后竀recordcode 字段一致，用于 lookup/delete/batchDelete) */
  recordcode: string
  /** 出库记录编码 (OneToOne ↀam_out_asset.recordcode, PK) */
  outasset_recordcode: string
  /** 回收资产编码 (FK ↀam_asset.recordcode) */
  recycle_asset: string
  /** 回收数量 */
  recycle_asset_number: number
  /** 回收仓库编码 (FK ↀam_storage.storage_code) */
  recycle_asset_storage_code: string
  /** 回收人工号(FK ↀemployee_jobcode，序列化器中通过 write_only 字段 recycle_asset_recycle_person_jobcode 映射) */
  recycle_asset_recycle_person_jobcode: string
  /** 回收日期 */
  recycle_asset_date: string
  /** 回收原因 */
  ...
  /** 回收描述 */
  recycle_asset_description: string | null
  /** 操作人工号(FK ↀemployee_jobcode，nullable) */
 ...
 * 回收资产列表响应接口
 */
export interface RecycleAssetListResponse {
  /** 总记录数 */
  count: number
  /** 下一页链掀*/
  next: string | null
...
export interface RecycleAssetBatchItem {
  /** 出库记录编码 (ↀrecycle_outasset_code, SlugRelatedField) */
...
export interface RecycleAssetBatchCreateForm {
  ...
  /** 回收仓库编码 (顶层共享字段, ↀrecycle_asset_storage) */
  recycle_asset_storage: string
  /** 回收人工号(顶层共享字段, ↀrecycle_asset_recycle_person_jobcode) */
  recycle_asset_recycle_person_jobcode: string
}
