/**
 * 资产管理 Store
 * 基于 createEntityStore 工厂创建，支持缓存、防重、分�? * 扩展自定义搜索方�?searchAssets（用于复杂条件搜索）
 */
...
interface AssetStore extends EntityStore<AssetDetail, PaginationQuery> {
   ...
   * @param params 搜索参数（支持模糊匹配和精确匹配�?   * @returns 资产列表响应（包�?count �?results�?   */
...
export const useAssetStore = (): AssetStore => {
  ...
  if (!('searchAssets' in store)) {
    ...
    /**
     * 自定义资产搜索方�?     * 使用后端 search_assets action，支持多条件组合搜索
     * �?getList({ search }) 更安全：
     * - 使用专用选择�?AssetSelector.search_assets()
     * - 显式过滤 is_deleted=False
     * - 预加载关联信�?     * - 参数白名单验�?     */
    extendedStore.searchAssets = async (
      params: PaginationQuery & Record<string, string | number>,
    ) => {
      // �?search 参数映射�?keyword（后�?search action 使用 keyword 参数名）
