 ...
 * @description 根据回收资产详情数据及其关联数据，生�?5 �?InfoCardConfig 配置对象�? * 用于驱动 InfoCard 组件展示基本信息、合同信息、使用人信息、回收人信息、仓库信息�? */
 ...
 * 【v1.1.0 对齐】移�?usingPerson（后端已删除 recycle_asset_using_person_jobcode），
 ...
 * @param data - 响应式数据源，包�?detail、contractDetail、usingPerson、recyclePerson、storageDetail
 * @returns 5 �?InfoCardConfig 计算属�? *
 * @example
 * ```ts
...
export function useRecycleAssetDetailCards(data: Ref<RecycleAssetDetailCardData>) {
   ...
   * 基本信息卡片配置
   *
   * 包含 8 个字段，双列网格布局�?   * - 左列：ID、资产编码、资产名称、规格型�?   * - 右列：回收时间、回收数量、回收描述、出库记录编�?   */
  const basicInfoConfig = computed<InfoCardConfig>(() => {
    ...
    return {
      ...
      fields: [
        [
          { label: '回收标识�?, value: d?.recordcode, defaultValue: 'N/A' },
          { label: '出库标识�?, value: d?.outasset_recordcode, defaultValue: 'N/A' },
          { label: '资产�?, value: d?.asset?.asset_code, defaultValue: 'N/A' },
          { label: '资产名称', value: d?.asset?.asset_name, defaultValue: 'N/A' },
          { label: '规格型号', value: d?.asset?.asset_specification, defaultValue: '�? },
        ],
        [
          {
            label: '回收时间',
          ...
          { label: '回收描述', value: d?.recycle_asset_description, defaultValue: '�? },
   ...
   * - 右列：签订日期、保修期、初验日期、终验日�?   * 数据来源：通过 assetAPI.getContractByAssetCode 直接获取�?Contract 对象
   */
    ...
    return {
      ...
      fields: [
        [
          ...
          { label: '合同供应�?, value: c?.contract_supplier, defaultValue: 'N/A' },
        ],
        [
          {
            label: '签订日期',
   ...
   * 【v1.1.0 对齐】数据来源改�?detail 中的 read_only 字段（using_person_name / using_person_jobcode），
   * 后端通过 FK 链自动获取：outasset_recordcode �?outasset_code �?asset_applicant_jobcode
   */
  const usingPersonInfoConfig = computed<InfoCardConfig>(() => {
    ...
    return {
      ...
      icon: 'User',
      fields: [
        [{ label: '使用�?, value: d?.employee_name, defaultValue: 'N/A' }],
        [{ label: '使用人工�?, value: d?.employee_jobcode, defaultValue: 'N/A' }],
        [{ label: '使用人部�?, value: d?.employee_department?.department_name, defaultValue: 'N/A' }],
      ],
    }
  })
  ...
  /**
   * 回收人信息卡片配�?   *
   * 包含 2 个字段，单列布局：回收人（姓�?部门）、回收人工号
   * 数据来源：通过 userStore.getById 获取�?EmployeeExtended 对象
   */
  const recyclePersonInfoConfig = computed<InfoCardConfig>(() => {
    ...
    return {
      ...
      fields: [
        ...
        [{ label: '回收人工�?, value: d?.recycle_person_jobcode, defaultValue: 'N/A' }],
        [{ label: '回收人部�?, value: d?.recycle_person_department, defaultValue: 'N/A' }],
      ],
    }
  })
   ...
   * - 右列：仓库类型、仓库描�?   * 数据来源：通过 storageStore.getById 获取�?Storage 对象
   */
    ...
    return {
      ...
      fields: [
        [
          ...
          { label: '仓库地址', value: s?.storage_address, defaultValue: '�? },
        ],
        [
          { label: '仓库类型', value: s?.storage_type, defaultValue: '�? },
          { label: '仓库描述', value: s?.storage_description, defaultValue: '�? },
        ],
      ],
    }
  })
