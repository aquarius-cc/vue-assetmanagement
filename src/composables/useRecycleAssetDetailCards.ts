/**
 * @file 回收资产详情卡片配置（基本信息/合同/使用人/回收人/仓库）
 * @module composables/useRecycleAssetDetailCards
 * @exports
 *   - useRecycleAssetDetailCards: 回收资产详情卡片 composable
 *   - RecycleAssetDetailCardData: 数据源接口
 * @callers
 *   - components/componentsdetails/detils/RecycleAssetBasicDetails.vue
 * @dependsOn
 *   - types/info-card | types/recycleasset | types/user
 *   - types/contract | types/storage | utils/Format
 */
 ...
 * 【v1.1.0 对齐】移除 usingPerson（后端已删除 recycle_asset_using_person_jobcode），
...
export function useRecycleAssetDetailCards(data: Ref<RecycleAssetDetailCardData>) {
    ...
    return {
      ...
      fields: [
        [
          ...
          { label: '合同金额', value: c?.contract_amount, isPrice: true },
          { label: '合同供应商', value: c?.supplier_name, defaultValue: 'N/A' },
        ],
        [
          {
            label: '签订日期',
          ...
          {
            label: '初验日期',
            value: c?.initial_check_date,
            formatter: (v) => formatDate(v as string) ?? '',
          },
          {
            label: '终验日期',
            value: c?.final_check_date,
            formatter: (v) => formatDate(v as string) ?? '',
          },
        ],
      ],
   ...
   * 【v1.1.0 对齐】数据来源改为 detail 中的 read_only 字段（using_person_name / using_person_jobcode），
  ...
  const recyclePersonInfoConfig = computed<InfoCardConfig>(() => {
    ...
    return {
      ...
      fields: [
        ...
        [{ label: '回收人部门', value: d?.recycle_person_department, defaultValue: 'N/A' }],
      ],
    }
  })
