 ...
 * @description 根据出库资产详情数据，生�?4 �?InfoCardConfig 配置对象�? * 用于驱动 InfoCard 组件展示基本信息、合同信息、申请人信息、保管人信息�? */
...
/**
 * 获取资产状态文�? *
 * @param value 状态值（�?'in_use', 'returned' 等）
 * @returns 可读的中文状态，空值返�?'未知'
 */
 ...
 * @param data - 响应式数据源，包�?detail、applicantUser、managerUser、assetContract
 * @returns 4 �?InfoCardConfig 计算属�? *
 * @example
 * ```ts
...
export function useOutAssetDetailCards(data: Ref<OutAssetDetailCardData>) {
   ...
   * 基本信息卡片配置
   *
   * 包含 11 个字段，双列网格布局�?   * - 左列：ID、出库唯一标识码、资产码、出库时间、资产状态、使用地�?   * - 右列：资产名称、规格型号、归还日期、出库类型、备注描�?   */
  const basicInfoConfig = computed<InfoCardConfig>(() => {
    ...
    return {
      ...
      fields: [
        [
          { label: '出库唯一标识�?, value: d?.recordcode, defaultValue: 'N/A' },
          { label: '入库唯一标识�?, value: d?.asset_recordcode, defaultValue: 'N/A' },
          { label: '资产�?, value: d?.asset_code },
          {
            label: '出库时间',
          ...
          { label: '使用地点', value: d?.using_location, defaultValue: '�? },
        ],
        [
          { label: '资产名称', value: d?.asset_name, defaultValue: '�? },
          { label: '规格型号', value: d?.asset_specification, defaultValue: '�? },
          {
            label: '归还日期',
            value: d?.return_date,
            formatter: (v) => formatDate(v as string) ?? '�?,
          ...
          { label: '备注描述', value: d?.outasset_description, defaultValue: '�? },
        ],
      ],
    }
  })
    ...
    return {
      ...
      fields: [
        [{ label: '所在合同号', value: c?.contract_code, defaultValue: 'N/A' },
          { label: '供应�?, value: c?.contract_supplier, defaultValue: 'N/A' }],
        [{ label: '所在合同名�?, value: c?.contract_name, defaultValue: 'N/A' },
  ...
  const applicantInfoConfig = computed<InfoCardConfig>(() => {
    ...
    return {
      ...
      fields: [
        ...
        [{ label: '申请人工�?, value: d?.outasset_applicant?.employee_jobcode || d?.outasset_applicant?.employee_jobcode, defaultValue: 'N/A' }],
      ],
    }
  })
  ...
  const managerInfoConfig = computed<InfoCardConfig>(() => {
    ...
    return {
      ...
      fields: [
        ...
        [{ label: '保管人工�?, value: d?.outasset_manager?.employee_jobcode || d?.outasset_manager?.employee_jobcode, defaultValue: 'N/A' }],
      ],
    }
  })
