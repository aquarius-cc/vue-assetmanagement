 ...
 * 用于创建回收资产时的表单数据
 *
 * 字段映射（前�?�?后端 RecycleAssetSerializer）：
...
export interface RecycleAssetCreateForm {
  ...
  /** 回收仓库编码 (�?recycle_asset_storage, SlugRelatedField) */
  recycle_asset_storage_code: string
  /** 回收人工�?(�?recycle_asset_recycle_person_jobcode, SlugRelatedField) */
 ...
 * 使用人信息通过序列化器 FK �?read_only 字段自动获取（using_person_name / using_person_jobcode�? */
export interface RecycleAsset {
  /** 主键 ID */
  id: number
  /** 后端记录编码 (与后�?recordcode 字段一致，用于 lookup/delete/batchDelete) */
  recordcode: string
  ...
  recycle_asset_storage_code: string
  /** 回收人工�?(FK �?employee_jobcode，序列化器中通过 write_only 字段 recycle_asset_recycle_person_jobcode 映射) */
  recycle_asset_recycle_person_jobcode: string
  /** 回收日期 */
  recycle_asset_date: string
  /** 回收原因 */
  ...
  /** 回收描述 */
  recycle_asset_description: string | null
  /** 操作人工�?(FK �?employee_jobcode，nullable) */
...
export interface RecycleAssetQueryParams {
  ...
  /** 排序字段 */
  ordering?: string
  /** 索引签名：允许任�?string key，但值必须是 string/number/boolean/null/undefined */
...
export interface RecycleAssetBatchCreateForm {
  ...
  /** 回收人工�?(顶层共享字段, �?recycle_asset_recycle_person_jobcode) */
  recycle_asset_recycle_person_jobcode: string
}
