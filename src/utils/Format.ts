...
// 日期格式化函数（修复类型错误�?const formatDate = (date: Date | number | string | null | undefined): string | null => {
  // 处理空值情�?  if (!date) return null
  ...
  if (date instanceof Date) {
    ...
    // 处理时间戳（数字类型）：13位毫秒级�?0位秒�?    const timestamp = date.toString().length === 10 ? date * 1000 : date
    dateObj = new Date(timestamp)
  } else if (typeof date === 'string') {
    // 处理日期字符�?    dateObj = new Date(date)
  } else {
    // 其他未知类型
    return null
  }
...
const formatPrice = (price: number | string | null | undefined): string => {
  // 1. 先将输入转为数字（处理字符串格式的金额，�?"1000" �?"1000.5"�?  const num = typeof price === 'number' ? price : Number(price)
  // 2. 处理非数字、NaN �?null 的情况，返回默认�?  if (isNaN(num) || price === null || price === undefined) {
...
// 格式化数字为千分位（修复类型错误�?const formatNumber = (num: number | string | null | undefined) => {
  // 处理空值情�?  if (num === undefined || num === null || num === '') return '0'

  // 将输入转换为数字（处理字符串类型的数字，�?"1234"�?  const number = typeof num === 'number' ? num : Number(num)
...
const contractiInfoFormate = (
  ...
  // �?rawData 为空（undefined �?null），返回空对象（或抛出错误，根据业务需求）
  if (!rawData) {
  ...
  const getSettlementStatus = (value: unknown): ContractSettlementStatus => {
    ...
    // 校验是否为合法的枚举值（利用 Object.values�?    const validStatuses = Object.values(ContractSettlementStatus); // ['pending', 'settled']
  ...
  // 格式�?+ 修正字段�?+ 确保类型正确
  // 第二步：格式化数据（确保 rawData 是对象类型，可安�?spread�?  const formattedData = {
    ...rawData,
    ...
    // 数字字段处理（兼�?undefined，避�?Number(undefined) 转为 NaN�?    contract_price: rawData.contract_price !== null ? Number(rawData.contract_price) : undefined,
    ...
    contract_settledment_price: rawData.contract_settledment_price !== null
      ...
      : null,
    contract_paid_price: rawData.contract_paid_price !== null ? Number(rawData.contract_paid_price) : 0, // 默认�?0
    contract_paid_count_number: rawData.contract_paid_count_number !== null
      ? Number(rawData.contract_paid_count_number)
      : 0, // 默认�?0
    // 字符串字段处理（兼容 undefined，避�?String(undefined) 转为 "undefined"�?    contract_type: rawData.contract_type !== null ? String(rawData.contract_type).trim() : undefined,
...
//   else if (data === 'information_construction') result.value = '信息化建设合�?
//   else if (data === 'direct_procurement') result.value = '直接采购合同'
//   else if (data === 'pending') result.value = '待结�?
//   else if (data === 'settled') result.value = '已结�?
...
/**
 * 根据英文状态获取中文显示文�? * @param status - 英文状态（可能�?undefined �?null�? * @returns 中文状态，若未匹配则返�?'�?
 */
export function getAssetStatusText(status?: string | null): string {
  if (!status) return '�?
  return assetCurrentStatusMapping[status as AssetCurrentStatus] ?? '�?
}

// 资产状态映射（作为assetCurrentStatusMapping的别名）
const assetStatusMapping: Record<string, string> = assetCurrentStatusMapping

// // 资产使用状态映�?// const assetUsingStatusMapping: Record<string, string> = {
//   idle: '闲置',
//   in_use: '在用',
//   under_maintenance: '维修�?,
//   out_of_service: '停用',
//   allocated: '已分�?,
//   pending_allocation: '待分�?,
// }

const contractTypeMapping: Record<string, string> = {
  purchase: '采购合同',
  service: '服务合同',
  information_construction: '信息化建设合�?,
...
const USER_STATUS_INPUT_MAPPING: Record<string, EmployeeStatus> = {
  ...
  退�? EmployeeStatus.RETIREMENT,
  在职员工: EmployeeStatus.ACTIVE,
  离职员工: EmployeeStatus.LEFT,
  退休员�? EmployeeStatus.RETIREMENT,
...
const USER_STATUS_DISPLAY_MAPPING: Record<EmployeeStatus, string> = {
  ...
  [EmployeeStatus.RETIREMENT]: '退�?,

}
// 🌟 新增：验证用户状态合法�?const validateUserStatus = (status: string): EmployeeStatus | null => {
...
const transformAndValidateExcelUser = (
  ...
  departmentCodeMap: Record<string, string>, // 部门代码→部门名称映�?): ValidatedEmployeeData => {
  ...
  // 校验并转换状�?  const statusValue = excelData.状�??? ''
  const validStatus = validateUserStatus(statusValue)
  if (!validStatus) {
    result.validationStatus = 'error'
    result.validationError = `状�?{statusValue}不合法（仅支持：在职/离职/退休）`
    return result
  }
...
const transformAndValidateExcelDepartment = (
  ...
  const result: ValidatedDepartmentData = {
    ...
    department_information: excelData.部门信息�?|| '',
    validationStatus: 'success',
    validationError: '',
  }
...
export function getOutAssetStatusText(status?: string | null): string {
  if (!status) return '�?
  return outassetStatusMapping[status as OutAssetCurrentStatus] ?? '�?
}
...
// 创建反向映射，将中文状态值转换为英文状态�?// const reverseStatusMapping: Record<string, string> = {
//   在职员工: 'active',
//   离职员工: 'left',
//   退休员�? 'retirement',
// }
...
const parseExcelDate = (dateStr: string): string | null => {
  ...
  if (!dateStr) {
    // 返回一个默认日期或空字符串，具体取决于后端API的要�?    // 如果后端允许空日期，可以返回空字符串；否则返回一个默认日�?    return '' // 或�?return '1970-01-01' 作为默认�?  }

  // 如果已经�?YYYY-MM-DD 格式，直接返�?  const dateRegex = /^\d{4}-\d{2}-\d{2}$/
  if (dateRegex.test(dateStr)) {
    return dateStr
  }

  // 尝试解析各种可能的日期格�?  let date: Date | null = null

  // 尝试解析 MM/DD/YYYY �?MM-DD-YYYY 格式
