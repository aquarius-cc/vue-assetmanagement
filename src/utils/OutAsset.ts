...
import type { Employee } from '@/utils/User'
...
export interface OutAssetCreateForm {
  /** 出库资产编码 (外键关联 am_asset.asset_code, 可�? */
  ...
  /** 关联合同编码 (外键关联 am_contract.contract_code, 可�? */
...
export interface OutAsset extends OutAssetCreateForm {
  ...
  /** 出库申请人工�?*/
  outasset_applicant_jobcode: string | null
  // [HR-01] 后端 v1.1.0 改为 read_only，但 OutAsset 响应接口仍保留此字段（后端仍返回�?  /** 出库保管人工�?*/
...
/**
 * 员工自动完成项接�? * 用于 el-autocomplete 下拉建议展示
 */
...
/**
 * 出库资产扩展表单接口
 * 仅用于前端表单展示，扩展�?outasset_name 和员工姓名字段�? * [HR-02] 后端 v1.2.0 恢复 applicant_name/manager_name 用于前端展示�? */
